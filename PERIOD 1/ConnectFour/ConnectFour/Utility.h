	...
	while (isRunning) { /// i always true. Need to fix. Here is the program
		...
		case CONNECT4GAME:
			...
			if (temp == "") {
				...
				int color11 = (temp == "CYAN") ? CYAN : (temp == "RED") ? RED : WHITE;
				setbkcolor(RED);
				setcolor(BLUE);
				bar(0, 0, maxX, maxY);
				if (color11 != WHITE) {
					...
					temp += "!";
					drawDRAW(maxX / 2, maxY / 2, 300, RED, RED);
				}
