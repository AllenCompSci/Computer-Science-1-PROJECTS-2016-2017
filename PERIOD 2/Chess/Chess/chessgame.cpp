	...
	void offBoard() {
		...
		if (currPiece == King) {
			...
			if (MATE) {
				...
				settextstyle(6, 0, 8);
				/// cout << "checkmate text" << textheight(str.c_str()) << endl;
				/// cout << "checkmate text" << textwidth(str.c_str()) << endl;
				for (int x = 585; x <= 1336; x++) {
					for (int y = 454; y <= 564; y++) {
						putpixel(x, y, 4);
					}
				}
				outtextxy(595, 464, str.c_str());
				// rectangle(575, 444, 1346, 574); 
				Sleep(2000);
				cleardevice();

				// if statement for white ot black winner based on turn
				if (Turn == Black) {
					setcolor(15);
					string str = "BLACK WINS";
					settextstyle(6, 0, 8);
					/// cout << "winner text black" << textheight(str.c_str()) << endl;
					/// cout << "winner text black" << textwidth(str.c_str()) << endl;
					outtextxy(597, 464, str.c_str());
					Sleep(1500);
					cleardevice();
					setcolor(4);
					string rstr = "GAME OVER";
					settextstyle(6, 0, 9);
					/// cout << "endgame text" << textheight(rstr.c_str()) << endl;
					/// cout << "endgame text" << textwidth(rstr.c_str()) << endl;
					outtextxy(539, 455, rstr.c_str());
					Sleep(1500);
					cleardevice();
				}
				else {
					...
					settextstyle(6, 0, 8);
					/// cout << "winner text white" << textheight(str.c_str()) << endl;
					/// cout << "winner text white" << textwidth(str.c_str()) << endl;
					outtextxy(606, 464, str.c_str());
					Sleep(1500);
					cleardevice();
					setcolor(4);
					string rstr = "GAME OVER";
...
*/
	...
	string str = "CHESS ";
	settextstyle(6, 0, 8);
	cout << "chess text" << textheight(str.c_str()) << endl;
	cout << "chess text" << textwidth(str.c_str()) << endl;
	outtextxy(763, 10, str.c_str());
...
void Chess() {
	...
	while (isRunning) {
		if (NewGame) {
			...
			// Opening Screen
			setcolor(4);
			string str = "WELCOME TO CHESS";
			settextstyle(6, 0, 9);
			/// cout << "chess opening screen x " << textwidth(str.c_str()) << endl;
			/// cout << "chess opening screen y " << textheight(str.c_str()) << endl;
			outtextxy(248, 356, str.c_str());


			string qstr = "LET'S PLAY";
			settextstyle(6, 0, 6);
			/// cout << "let's play x " << textwidth(qstr.c_str()) << endl;
			/// cout << "let's play y " << textheight(qstr.c_str()) << endl;
			outtextxy(751, 480, qstr.c_str());

			line(701, 510, 248, 510);
			line(1220, 510, 1672, 510);


			string dstr = "PRESS SPACEBAR";
			settextstyle(6, 0, 4);
			cout << "spacebar x " << textwidth(dstr.c_str()) << endl;
			cout << "spacebar y " << textheight(dstr.c_str()) << endl;
		...
		if (EndTurn) {
			...
			if (Turn == White) {
				setcolor(15);
				string str = "WHITE TURN ";
				settextstyle(6, 0, 5);
				cout << textheight(str.c_str()) << endl;
				cout << textwidth(str.c_str()) << endl;
				outtextxy(755, 940, str.c_str());
			}
			else if (Turn == Black) {
				setcolor(04);
				string str = "BLACK TURN "; 
				settextstyle(6, 0, 5);
				cout << textheight(str.c_str()) << endl;
				cout << textwidth(str.c_str()) << endl;
				outtextxy(752, 940, str.c_str());
			}
			for (int r = 0; r < 8; r++) {
				for (int c = 0; c < 8; c++) {
			...
			if (CHECK) {
				...
				while (!kbhit()) {
					if (flash)
						...
						setcolor(15);
					flash = !flash;
					// rectangle(741, 444, 1179, 574);
					outtextxy(761, 10, str.c_str());
					Sleep(400);
				}
				...
				string sstr = "CHESS ";
				settextstyle(6, 0, 8);
				cout << "chess text" << textheight(sstr.c_str()) << endl;
				cout << "chess text" << textwidth(sstr.c_str()) << endl;
				outtextxy(763, 10, sstr.c_str());
				// putpixel(0, 0, 0);
